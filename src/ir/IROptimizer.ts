...
export class IROptimizer {
  ...
  constPropAndFold() {
    ...
    for (let eqVar of eqVars) {
      let constStk: string[] = [],
        nodeStk: { type: 'op' | 'var'; name: string; i: number }[] = []
      ...
      while (nodeStk.length) {
        ...
        if (node.type == 'var') {
          ...
          let args = []
          for (let i = 0; i < node.i; i++) args.push(constStk.pop())
          switch (node.name) {
            case 'OR_OP':
            ...
            case 'LE_OP':
              ...
              break
            case 'PLUS':
              if (node.i == 2) constStk.push((Number(args[0]) + Number(args[1])).toString())
              else constStk.push(Number(args[0]).toString())
              break
            case 'MINUS':
              if (node.i == 2) constStk.push((Number(args[0]) - Number(args[1])).toString())
              else constStk.push((-Number(args[0])).toString())
      ...
      if (optimizable) {
        let newQuad = new Quad('=const', constStk[0], '', eqVar.v.res)
        unfix = true
      ...
      function optimArg1() {
        ...
        if (record.arg1.optimizable && record.arg1.constant == '0') {
          switch (v.op) {
            case 'PLUS':
              // 0 + a = a
              modify(new Quad('=var', quad.arg2, '', quad.res))
              break
            case 'MINUS':
              // 0 - a = -a
            ...
            case 'MULTIPLY':
              // 0 * a = 0
              modify(new Quad('=const', '0', '', quad.res))
              break
            case 'SLASH':
              // 0 / a = 0
              modify(new Quad('=const', '0', '', quad.res))
      ...
      function optimArg2() {
        ...
        if (record.arg2.optimizable && record.arg2.constant == '0') {
          switch (v.op) {
            case 'PLUS':
              // a + 0 = a
              modify(new Quad('=var', quad.arg1, '', quad.res))
              break
            case 'MINUS':
              // a - 0 = a
              modify(new Quad('=var', quad.arg1, '', quad.res))
