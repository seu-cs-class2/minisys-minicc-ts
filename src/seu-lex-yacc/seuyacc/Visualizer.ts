 ...
 * 可视化LR0自动机
 */
export function visualizeLR0GOTOGraph(lr0dfa: LR0DFA, lr0Analyzer: LR0Analyzer, viewNow = true) {
  let dumpObject: {
    nodes: {
      key: string
      label: string
  ...
  // 设置点（项目集）
  for (let i = 0; i < lr0dfa.states.length; i++) {
    let topPart = `I${i}\n=======\n`,
      stateLines = [],
      kernelItem = true
    for (let item of lr0dfa.states[i].items) {
      let leftPart = ''
      leftPart += lr0Analyzer.symbols[item.rawProducer.lhs].content
      leftPart += ' -> '
      let j = 0
      for (; j < item.rawProducer.rhs.length; j++) {
        if (j == item.dotPosition) leftPart += '●'
        leftPart += lr0Analyzer.getSymbolString(item.rawProducer.rhs[j]) + ' '
      }
      if (j == item.dotPosition) leftPart = leftPart.substring(0, leftPart.length - 1) + '●'
      let sameLeftPos = stateLines.findIndex(x => x.leftPart == leftPart)
      if (sameLeftPos === -1) {
    ...
    let stateString = topPart
    stateLines.forEach(v => {
      stateString += v.leftPart + '\n'
    })
    dumpObject.nodes.push({ key: `K${i}`, label: stateString.trim(), color: '#FFFFFF' })
  }
  // 设置边（迁移）
  for (let i = 0; i < lr0dfa.states.length; i++) {
    lr0dfa.adjList[i].forEach(x => {
      dumpObject.edges.push({
        source: `K${i}`,
        target: `K${x.to}`,
        name: `K${i}_${x.to}`,
        label: lr0Analyzer.symbols[x.alpha].content,
      })
    })
  }
  let dagreJSON = JSON.stringify(dumpObject, null, 2)
...
/**
 * 可视化LALR分析表（ACTIONGOTOTable）
 */
export function visualizeLALRACTIONGOTOTable(lr1Analyzer: LR1Analyzer, viewNow = true) {
  visualizeLR1ACTIONGOTOTable(lr1Analyzer, viewNow)
}
...
export function visualizeLALRGOTOGraph(lalrdfa: LALRDFA, lr1Analyzer: LALRAnalyzer, viewNow = true) {
  ...
  for (let i = 0; i < lalrdfa.states.length; i++) {
    let topPart = `I${i}\n=======\n`,
      stateLines = [],
      kernelItem = true
    for (let item of lalrdfa.states[i].items) {
      let leftPart = ''
      leftPart += lr1Analyzer.symbols[item.rawProducer.lhs].content
      leftPart += ' -> '
      let j = 0
